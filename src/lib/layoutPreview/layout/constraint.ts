...
export const layoutConstraint: ContainerFn = (node, boxW, boxH, maxW, maxH, place, ctx) => {
  ...
  const dpx = (c: LNode, k: string) => { const x = resolveDimen(c.attrs[k], ctx); return x.mode === "fixed" ? x.px : 0; };
  const m = children.map((c) => nodeMargins(c, ctx));

  const wMode = children.map((c) => resolveDimen(c.attrs.layout_width, ctx).mode);
  const hMode = children.map((c) => resolveDimen(c.attrs.layout_height, ctx).mode);
